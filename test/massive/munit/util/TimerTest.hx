...
class TimerTest {
	...
	@AsyncTest
	public function testConstructor(factory:AsyncFactory) {
	...
	function delayedHandler() Timer.delay(handler, 1);

	function onRepeatedTimer() Assert.isTrue(count > 1);

	@AsyncTest
	public function shouldStopTimer(factory:AsyncFactory) {
	...
	function onStoppedTimer() Assert.areEqual(0, count);

	@AsyncTest
	public function shouldDelayAndCallOnce(factory:AsyncFactory) {
		handler = factory.createHandler(this, onDelayedTimer);
		instance = Timer.delay(timerHandler, 10);
		Timer.delay(handler, 200);
	}

	function onDelayedTimer() Assert.areEqual(1, count);

	@AsyncTest
	public function shouldIncrementStamp(factory:AsyncFactory) {
